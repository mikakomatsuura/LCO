...
dir = '/Users/mikako/work/teaching/2025_ObsTech/LCO/data/NGC3201/NGC3201_B/'
...
ccd1 = CCDData(hdu_list[0].data, meta=hdu_list[0].header, unit=u.adu, wcs=WCS(hdu_list[0].header))
...
hdr['HISTORY'] = 'Sigma-clipped average combination from frames 1-4.'
...
for key in ['EXPTIME', 'EXP_TIME', 'EXPOSURE', 'ITIME']:
        if key in hdr:
                try:
                        ...
                        hdr['HISTORY'] = f'{key} scaled by 4x for combined exposure.'
                except Exception:
                        pass
...
ax1.imshow(hdu_list[0].data, origin='lower', vmin=-1, vmax=20)
ax1.coords['ra'].set_axislabel('Right Ascension')
ax1.coords['dec'].set_axislabel('Declination')
ax1.set_title('Original (HDU1)')

ax2 = plt.subplot(1,2,2, projection=WCS(hdu_list[0].header))
ax2.imshow(final_image.data, origin='lower', vmin=-1, vmax=20)
